...
#print("\n--- IMPORTANCIAS BLOQUE 5 (CUANTI-CATEGÓRICAS) ---")
#print(imp5.head(10))
...
def preparar_datos_evaluacion(df_nuevo):
    ...
    base_dum_new = base_dum_new.reindex(columns=base_dum.columns, fill_value=0) 
...
def modelo_train_test(X, y):
    ...
    modelo1  = XGBClassifier(colsample_bytree=1, tree_method='hist', random_state=19052023)
    parametros1 = {'n_estimators':[100,200,300],
                   'max_depth':[2,3,5,7],
                   'min_child_weight':[15,20,30,40],
    ...
    imp1 = pd.DataFrame({'Variable':X_train.columns,
                  'Importancia':grilla1.best_estimator_.feature_importances_})
...
def modelo_test_train(X, y):
    ...
    modelo2  = XGBClassifier(colsample_bytree=1)
    parametros1 = {'n_estimators':[100,200,300],
               'max_depth':[2,3,5,7],
               'min_child_weight':[15,20,30,40],
               'learning_rate': [0.001,0.005,0.01,0.05,0.1,0.5]
...
def modelo_sin_balanceo(X,y):
    ...
    modelo_XG  = XGBClassifier(colsample_bytree=1, tree_method='hist', random_state=19052023)
    parametros1 = {
...
def modelo_x_balanceado(X,y):
    ...
    modelo2_XG  = XGBClassifier(colsample_bytree=1, tree_method='hist', random_state=19052023)
    parametros2 = {
        'n_estimators': [200, 300, 400],    
    'max_depth': [4, 6, 8],                 
    'min_child_weight': [5, 10, 15],        
...
def modelo_ensamble_sin_balanceo(X, y, mejor_xgb):
    
    T1 = preprocessing.MinMaxScaler().fit(X)
    X_Z = T1.transform(X)

    modelo_Logistica = LogisticRegression(C=1.0, max_iter=1000, random_state=20042021)
    Bagging_Logistica = BaggingClassifier(
        estimator=modelo_Logistica,
        n_estimators=20,
        max_samples=0.8,
        max_features=1.0,
        n_jobs=-1,
        random_state=20042021
    )

    Ensamble_BE = VotingClassifier(
        estimators=[
            ('bag_log', Bagging_Logistica),
            ('xgb', mejor_xgb) 
        ],
        voting='soft',
        n_jobs=-1
    ...
    print("\nEntrenando Ensamble Híbrido (Sin Balancear)...")
    Ensamble_BE = Ensamble_BE.fit(X_Z, y)
    
    predicciones_train(modelo=Ensamble_BE, X_train_Z=X_Z, y_udr=y)

    return Ensamble_BE, T1


def modelo_ensamble_balanceado(X, y, mejor_xgb_balanced):
    ...
    Ensamble_BE_Balanced = VotingClassifier(
        estimators=[
            ('bag_log', Bagging_Logistica),
            ('xgb', mejor_xgb_balanced)  
        ],
        voting='soft',
        n_jobs=-1
    )
    
    print("\nEntrenando Ensamble Híbrido (Balanceado con SMOTE)...")
    Ensamble_BE_Balanced = Ensamble_BE_Balanced.fit(X_Z_udr, y_udr)
    
    predicciones_train(modelo=Ensamble_BE_Balanced, X_train_Z=X_Z_udr, y_udr=y_udr)

    return Ensamble_BE_Balanced, T2, X_udr.columns
...
# Ensamble sin balanceo balanceado
modelo_ensamble_sb, escalador_ensamble_sin = modelo_ensamble_sin_balanceo(X, y, modelo_sin_ba)
# Ensamble balanceado
modelo_ensamble_b, escalador_ensamble_balanced, columnas_molde_ensamble_balanced = modelo_ensamble_balanceado(X, y, modelo_campeon)
...
X_prueba_Z = escalador_ensamble_balanced.transform(X_prueba_procesada)
X_prueba_Z_sin = escalador_ensamble_sin.transform(X_prueba_procesada)
...
#evaluacion_prueba_sin_balanceo.to_csv(path2 + "submission_Xboots_Sin_balanceo.csv", index=False)
#evaluacion_prueba_balanceo.to_csv(path2 + "submission_Xboots_Balanceo.csv", index=False)
evaluacion_prueba_sin_balanceo.to_csv(path2 + "submission_Ensamble_Sin_balanceo.csv", index=False)
evaluacion_prueba_balanceo.to_csv(path2 + "submission_Ensamble_Balanceo.csv", index=False)

#modelo_x_balanceado(X)