...
def modelo_train_test(X, y):
    ...
    modelo1  = LogisticRegression(max_iter=1000, random_state=19052023)
    parametros1 = {'C':[0.001, 0.01, 0.1, 1.0, 10.0, 100.0]}
...
def modelo_test_train(X, y):
    ...
    modelo2  = LogisticRegression(max_iter=1000, random_state=19052023)
    parametros1 = {'C':[0.001, 0.01, 0.1, 1.0, 10.0, 100.0]}
...
def modelo_sin_balanceo(X,y):
    ...
    modelo_XG  = LogisticRegression(max_iter=1000, random_state=19052023)
    parametros1 = {'C': [0.001, 0.01, 0.1, 1.0, 10.0, 100.0]}
...
def modelo_x_balanceado(X,y):
    ...
    modelo2_XG  = LogisticRegression(max_iter=1000, random_state=19052023)
    parametros2 = {'C': [0.001, 0.01, 0.1, 1.0, 10.0, 100.0]}
    auc_scorer = make_scorer(roc_auc_score, greater_is_better=True, response_method='predict_proba')
...
evaluacion_prueba_sin_balanceo.to_csv(path2 + "submission_Logistica_Sin_balanceo.csv", index=False)
evaluacion_prueba_balanceo.to_csv(path2 + "submission_Logistica_Balanceo.csv", index=False)
